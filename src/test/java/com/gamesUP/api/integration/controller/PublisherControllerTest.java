...
class PublisherControllerTest {
    ...
    void getAllPublishers_shouldReturnList() throws Exception {
        when(publisherService.getAllPublishers()).thenReturn(List.of(new PublisherDto(1L, "Publisher1")));

        mockMvc.perform(get("/publishers"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value(200))
                .andExpect(jsonPath("$.data[0].name").value("Publisher1"));
    }

    @Test
    void getPublisherById_shouldReturnPublisher() throws Exception {
        when(publisherService.getPublisherById(1L)).thenReturn(new PublisherDto(1L, "Publisher1"));

        mockMvc.perform(get("/publishers/1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value(200))
                .andExpect(jsonPath("$.data.name").value("Publisher1"));
    }
}
