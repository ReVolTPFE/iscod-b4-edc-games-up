...
public class GameController {
    ...
    public ResponseEntity<ApiResponse<List<GameDto>>> getAllGames() {
        ...
        return ResponseEntity.ok(new ApiResponse<>(200, "Games fetched successfully", games));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<GameDto>> getGameById(@PathVariable Long id) {
        GameDto game = gameService.getGameById(id);
        return ResponseEntity.ok(new ApiResponse<>(200, "Game fetched successfully", game));
    }
}
