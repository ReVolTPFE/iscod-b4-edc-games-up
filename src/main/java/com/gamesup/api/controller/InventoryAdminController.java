...
public class InventoryAdminController {
    ...
    public ResponseEntity<ApiResponse<List<InventoryDto>>> getAll() {
        ...
        return ResponseEntity.ok(new ApiResponse<>(200, "Inventories fetched successfully", inventories));
    }
    ...
    public ResponseEntity<ApiResponse<InventoryDto>> getById(@PathVariable Long id) {
        ...
        return ResponseEntity.ok(new ApiResponse<>(200, "Inventory fetched successfully", dto));
    }
    ...
    public ResponseEntity<ApiResponse<InventoryDto>> create(@RequestBody @Valid CreateInventoryDto dto) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new ApiResponse<>(201, "Inventory created successfully", created));
    }
    ...
    public ResponseEntity<ApiResponse<InventoryDto>> update(@PathVariable Long id, @RequestBody @Valid UpdateInventoryDto dto) {
        ...
        return ResponseEntity.ok(new ApiResponse<>(200, "Inventory updated successfully", updated));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<ApiResponse<String>> delete(@PathVariable Long id) {
        inventoryService.deleteInventory(id);
        return ResponseEntity.ok(new ApiResponse<>(200, "Inventory deleted successfully"));
    }
}
